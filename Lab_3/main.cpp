...
//  Modified by Tajmilur Rahman on 25/0126
...
bool isPhong = false;       // Defaulting to Gouraud based on your snippet
bool isWireframe = false;   // Defaulting to Fill mode
...
int main()
{
    ...
    while (!glfwWindowShouldClose(window))
    {
        ...
        Shader& lightingShader = isPhong ? phongShader : gouraudShader;
...
void drawCube(unsigned int& cubeVAO, Shader& lightingShader, glm::vec3 color, glm::vec3 pos, glm::vec3 rot, glm::vec3 scale)
{
    ...
    lightingShader.setVec3("material.ambient", glm::vec3(0.4f, 0.9f, 0.8f) * 0.50f);
...
void key_callback(GLFWwindow* window, int key, int scancode, int action, int mods)
{
    ...
    // Toggle Shader (Phong vs Gouraud)
    if (key == GLFW_KEY_0 && action == GLFW_PRESS)
    {
        isPhong = !isPhong;
    }

    // Toggle Polygon Mode (Wireframe vs Fill)
