...
float lampRotation = 0.0f;    // Key H to rotate lamp head
float lampArmRotation = 0.0f; // Key P to rotate lamp arm
...
void printUsage() {
    ...
    cout << "H - Rotate hanging lamp" << endl;
    cout << "P - Rotate lamp arm" << endl;
...
void render() {
    ...
    // 6. TABLE LAMP (floating in the air - simple approach)
    // Position: floating in the air in the room
    hangingLamp->draw(*ourShader, identity, 1.5f, 1.5f, 1.0f, lampRotation, lampArmRotation);
}
...
void processInput(GLFWwindow* window, float deltaTime)
{
    ...
    // Lamp Rotation Control (Key H)
    if (glfwGetKey(window, GLFW_KEY_H) == GLFW_PRESS) {
        lampRotation += 100.0f * deltaTime;
        if (lampRotation > 360.0f) lampRotation -= 360.0f;
    }

    // Lamp Arm Rotation Control (Key P)
    if (glfwGetKey(window, GLFW_KEY_P) == GLFW_PRESS) {
        lampArmRotation += 100.0f * deltaTime;
        if (lampArmRotation > 360.0f) lampArmRotation -= 360.0f;
    }
